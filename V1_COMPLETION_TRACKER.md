...
| Qdrant | Source-wired; target VPS proof required | Partially | RAG/memory/research | `docker run -d --name amarktai-qdrant --restart unless-stopped -p 127.0.0.1:6333:6333 -v /var/www/amarktai/qdrant:/qdrant/storage qdrant/qdrant:latest; export QDRANT_URL=http://127.0.0.1:6333` | Requires Docker/service proof on target VPS |
...
| Scrapy | Source-wired; target VPS proof required | Source-wired | Web research/scrape | `python3 -m venv /var/www/amarktai/.venv && /var/www/amarktai/.venv/bin/pip install scrapy trafilatura && export AMARKTAI_PYTHON_BIN=/var/www/amarktai/.venv/bin/python` | Proof must import Scrapy with `AMARKTAI_PYTHON_BIN` when configured |
| Trafilatura | Source-wired; target VPS proof required | Source-wired | Web extraction | `python3 -m venv /var/www/amarktai/.venv && /var/www/amarktai/.venv/bin/pip install scrapy trafilatura && export AMARKTAI_PYTHON_BIN=/var/www/amarktai/.venv/bin/python` | Proof must import Trafilatura with `AMARKTAI_PYTHON_BIN` when configured |
...
| Local artifact storage | Source-wired; target VPS proof required | Requires `STORAGE_DRIVER=local_vps` and `AMARKTAI_STORAGE_ROOT` | Durable preview/download | Ensure `AMARKTAI_STORAGE_ROOT=/var/www/amarktai/storage` exists and is writable | Proof now uses production storage verification, so target VPS permissions must pass |
