...
import { CreateJobRequestSchema, hasBlockedOverrides, parseBearerToken, QUEUE_NAMES, DEFAULT_JOB_OPTIONS, } from '@amarktai/core';
// ── Auth Helper ───────────────────────────────────────────────────────────────
async function authenticateAppKey(bearerHeader) {
    if (!bearerHeader) {
...
export async function jobRoutes(app) {
    ...
    function getQueue() {
        if (!queue) {
            if (!app.redis)
                throw new Error('Redis is required for job queue');
