...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 2026-06-17 | Phase 8 final stabilization | `integration/cline-source-of-truth` | `docs/deploy/backend-deploy-checklist.md`, `docs/CHANGELOG_AI.md`, `docs/CODEX_CONTEXT.md` | Tighten the deploy-readiness checklist only. The canonical deploy doc now points at the correct branch, VPS path, standalone static/public copy steps, canonical `amarktai-platform.service` and `amarktai-worker.service`, local/public health verification, CSS asset checks, and a concise rollback sequence. | None. | Deploy readiness only; provider truth unchanged. | VPS execution proof is still required after pulling the branch and running the checklist on the server. | Use the updated checklist on the VPS, then rerun the focused test/build proof and service restarts before any live smoke verification. |
| 2026-06-16 | #118 follow-up | `integration/cline-source-of-truth` | Dashboard capabilities/jobs backend connections, runtime truth tests, dashboard docs | Connect dashboard backend surfaces to Brain/runtime truth without redesigning the UI. The Capabilities page now reads the canonical capability truth API, the Jobs page now reads the canonical `/api/admin/jobs` surface, and focused tests lock the canonical nav, runtime-truth endpoint, legacy Providers positioning, and canonical jobs/artifacts backend connections. | Dashboard/runtime truth only; provider truth unchanged. | dashboard nav, capabilities, jobs, artifacts, runtime truth. | Studio, Settings, and some provider diagnostics still aggregate mixed runtime or compatibility views. Visual redesign and deeper parity cleanup remain future work. | Keep Providers diagnostic-only, continue moving dashboard surfaces onto canonical backend truth, and postpone any visual redesign until backend parity is stable. |
| 2026-06-16 | #118 follow-up | `integration/cline-source-of-truth` | Brain research route, admin research URL/reports contract tests, operating docs | Audit and tighten the Phase 2 research engine without redesigning the Brain architecture. `/api/brain/research` now exposes the canonical execution fields already produced by the capability runtime, admin URL research now makes ignored provider/model/endpoint preferences explicit, and focused tests cover Brain research routing, scrape capability shape, and research artifact/job reporting. | Brain research engine only; provider proof unchanged. | research, scrape_website, research_result artifacts, research admin jobs. | Live provider-backed research completion and any deeper multi-agent research workflow remain unproven here. Admin research assist/status remain diagnostic/operator surfaces rather than the app contract. | Keep apps capability-only, keep research routed through Brain capabilities, and only upgrade live completion claims after authenticated production proof. |
| 2026-06-16 | #118 follow-up | `integration/cline-source-of-truth` | MiMo provider truth, focused discovery/auth/runtime tests, operating docs | Audit and tighten MiMo completion truth without redesigning the runtime. Canonical MiMo provider truth now matches the proven repo surface: dynamic token-plan discovery, OpenAI-compatible text/chat, and MiMo-native TTS. Unproven MiMo STT, voice clone, media generation, async jobs, tool-calling, and artifact claims are kept conservative so empty MiMo catalogs cannot fabricate false provider-contract routes. | MiMo. | chat, reasoning, coding, TTS, discovery metadata. | No live MiMo provider proof is recorded here beyond prior chat/TTS smoke references. MiMo STT, vision, voice clone, media generation, and artifact claims are not proven canonical runtime contracts, and canonical Brain async polling still does not exist. | Keep apps capability-only, keep MiMo async truth narrowed, and only upgrade MiMo rows after authenticated live provider proof. |
