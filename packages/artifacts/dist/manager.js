...
import { isValidMimeForType, } from '@amarktai/core';
// ── Save Artifact ─────────────────────────────────────────────────────────────
export async function saveArtifact(opts) {
    ...
    const storageKey = storage.buildStorageKey(input.appSlug, input.type, filename);
    // Write to filesystem
    ...
    if (!isValidMimeForType(input.type, finalMime)) {
        // Log warning but don't fail — accept whatever was generated
        console.warn(`[artifacts] MIME ${finalMime} is not standard for type ${input.type}, accepting anyway`);
    }
    // Write to database
...
// ── Get Artifact File ─────────────────────────────────────────────────────────
export async function getArtifactFile(artifactId) {
