...
import { getStorageRoot, STORAGE_SUBDIRS, } from '@amarktai/core';
// ── Path Safety ───────────────────────────────────────────────────────────────
function assertInsideBase(basePath, key) {
...
export class ArtifactStorageDriver {
    ...
    async put(key, data, explicitMimeType) {
    ...
    buildStorageKey(appSlug, type, filename) {
...
export function getArtifactStorage() {
    if (!_driver)
        _driver = new ArtifactStorageDriver();
    return _driver;
}
