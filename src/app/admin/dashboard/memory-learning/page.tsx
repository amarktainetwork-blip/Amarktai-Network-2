...
export default function MemoryLearningPage() {
  ...
  const layers = [
    'user memory', 'app memory', 'agent memory', 'emotional state/context',
    'memory retrieval', 'knowledge/artifact linking', 'daily learning scheduler',
    'cross-agent knowledge sharing', 'local VPS storage first',
  ]
  ...
  return (
    <div className="space-y-5">
      ...
      <section className="relative overflow-hidden rounded-2xl border border-slate-700/50 bg-slate-900/60 p-5 backdrop-blur-xl lg:p-7">
        ...
        <h2 className="mt-2 text-2xl font-black tracking-tight text-slate-100 lg:text-3xl">Shared Superbrain memory.</h2>
        <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-400">
          User memory · app memory · agent memory · emotional context · learning logs · artifact linking — stored locally first.
        </p>
      </section>
      ...
      <section className="grid gap-4 xl:grid-cols-[0.85fr_1.15fr]">
        ...
        {/* Recent memory */}
        <div className="rounded-2xl border border-slate-700/50 bg-slate-900/60 p-5 backdrop-blur-xl">
          <h3 className="text-sm font-black text-slate-200">Recent memory</h3>
          <div className="mt-4 space-y-2.5">
            {memory.slice(-8).reverse().map((entry) => (
              <article key={entry.id} className="rounded-xl border border-slate-700/40 bg-slate-800/50 p-3.5">
                <p className="text-[10px] font-black uppercase tracking-[0.16em] text-cyan-400/70">{entry.appSlug ?? 'superbrain'} / {entry.memoryType ?? 'memory'}</p>
