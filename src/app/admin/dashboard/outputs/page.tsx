  ...
  return (
    <div className="space-y-5">
      <section className="rounded-3xl border border-cyan-400/20 bg-slate-900/70 p-6">
        ...
        <h1 className="mt-2 text-3xl font-black text-white">Everything the network creates.</h1>
        <p className="mt-2 text-sm leading-6 text-slate-400">Songs, movies, avatars, images, research, app builds, diffs, pull requests, and deployment results share one artifact library.</p>
      </section>

      {!artifacts.length ? (
        <section className="rounded-2xl border border-dashed border-slate-700 bg-slate-900/40 p-12 text-center">
          <p className="font-black text-slate-300">No outputs yet.</p>
          <p className="mt-2 text-sm text-slate-500">Run a command and saved artifacts will appear here.</p>
        </section>
      ) : (
          ...
          {artifacts.map((artifact) => {
            ...
            return (
              <article key={artifact.id} className="rounded-2xl border border-slate-700/50 bg-slate-900/60 p-4">
                ...
                {url && <a href={url} className="mt-3 inline-block text-xs font-black text-cyan-300">Open artifact</a>}
              </article>
            )
          })}
        </section>
