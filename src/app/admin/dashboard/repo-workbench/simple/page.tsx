...
import { ArrowLeft, CheckCircle2, Loader2, RefreshCw, ShieldCheck, Trash2, Wand2, XCircle } from 'lucide-react'
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between gap-3">
        <Link href="/admin/dashboard/repo-workbench" className="inline-flex items-center gap-2 text-xs text-slate-500 hover:text-cyan-300"><ArrowLeft className="h-3.5 w-3.5" /> Repo Workbench</Link>
      ...
      <section className="grid gap-4 lg:grid-cols-[0.85fr_1.15fr]">
        <div className="space-y-4">
          <Panel title="1. GitHub connection status">
            ...
            <p className="mt-1 text-xs text-slate-500">{github?.blocker || 'Settings/vault token is used automatically.'}</p>
          </Panel>

          <Panel title="2. Repo and branch">
            <label className="text-xs text-slate-500">Repo selector from connected GitHub account</label>
      ...
      <section className="grid gap-4 lg:grid-cols-3">
        <Panel title="5. Apply and checks">
          <button onClick={applyPatch} disabled={!workspace || !patchId || !approved || Boolean(loading)} className="btn-primary">Apply patch</button>
          <div className="mt-3 flex flex-wrap gap-2">
            {['lint', 'test', 'build'].map((cmd) => <button key={cmd} onClick={() => runCheck(cmd)} disabled={!workspace || Boolean(loading)} className="btn-secondary">Run {cmd}</button>)}
          </div>
        </Panel>
