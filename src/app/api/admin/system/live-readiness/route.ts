...
export async function GET(_request: NextRequest) {
  ...
  checks.push(
    ...
    check(
      ...
      image?.status === 'READY' ? 'PASS' : 'FAIL',
      image ? `status=${image.status}; models=${image.models.length}` : 'Image capability not returned by runtime truth.',
      image?.status === 'READY' ? null : image?.blocker ?? 'Configure and test an image-capable provider.',
    ),
    check(
      'video_generation',
      'Video generation available',
      video?.status === 'READY' ? 'PASS' : 'DISABLED',
      video ? `status=${video.status}; models=${video.models.length}` : 'Video capability not returned by runtime truth.',
      video?.status === 'READY' ? null : video?.blocker ?? 'Video remains disabled until a real provider route, quota and artifact flow are verified.',
    ),
    check(
      'voice_tts',
      'Voice/TTS available',
      voice?.status === 'READY' ? 'PASS' : 'FAIL',
      voice ? `status=${voice.status}; models=${voice.models.length}` : 'Voice capability not returned by runtime truth.',
      voice?.status === 'READY' ? null : voice?.blocker ?? 'Configure and test an approved voice-capable provider.',
    ),
    check(
      'music_generation',
      'Music generation available',
      music?.status === 'READY' ? 'PASS' : 'DISABLED',
      music ? `status=${music.status}; models=${music.models.length}` : 'Music capability not returned by runtime truth.',
      music?.status === 'READY' ? null : music?.blocker ?? 'Music stays disabled until a real route/provider is verified.',
    ),
    check(
      'adult_mode',
    ...
    check(
      ...
      adult?.status === 'READY' ? 'PASS' : 'DISABLED',
      adult ? `status=${adult.status}; models=${adult.models.length}` : 'Adult capability not returned by runtime truth.',
      adult?.status === 'READY' ? null : adult?.blocker ?? 'Adult image remains disabled until specialist provider test passes.',
    ),
    check(
      'web_research',
      'Web crawler/research capability',
      webCrawler?.status === 'READY' ? 'PASS' : 'OPTIONAL',
