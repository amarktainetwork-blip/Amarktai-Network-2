 ...
 * No approved active provider SDK currently processes the scenes into rendered
  ...
  try {
    ...
    if (Array.isArray(providedScenes) && providedScenes.length > 0) {
      return NextResponse.json({
        ...
        generation_blocker: 'No approved active video generation provider SDK is integrated.',
    ...
    // Prefer GenX when available, otherwise fall back to template planning.
    const sceneCount = Math.max(1, Math.min(6, Math.ceil(duration / 5)));
    const llmPrompt =
...
No markdown, no explanation — raw JSON array only.`;
    ...
    return NextResponse.json({
      ...
      message: aiGenerated
        ...
        : 'Video planning complete using rule-based scene decomposition (no approved active AI planning provider configured).',
      generation_available: false,
      generation_blocker: 'No approved active video generation provider SDK is integrated. Provider API key alone is not sufficient; the rendering pipeline must be implemented.',
      params: {
