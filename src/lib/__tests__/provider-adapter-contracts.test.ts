...
describe('provider adapter contracts', () => {
  ...
  it('classifies MiMo TTS and ASR families conservatively', () => {
    ...
    expect(models.find((model) => model.id === 'mimo-v2.5-asr')?.metadata?.executionClassification).toBe('blocked_by_policy')
    expect(models.find((model) => model.id === 'mimo-v2.5-asr')?.metadata?.executable).toBe('CATALOG_ONLY')
