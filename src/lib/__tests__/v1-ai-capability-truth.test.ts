...
describe('V1 universal AI capability truth', () => {
  ...
  it('keeps V1 proof classification truthful for unresolved Phase 2 provider capabilities', () => {
    ...
    expect(proof).toContain('SOURCE_WIRED_ON_MISSING_CREDENTIAL')
    expect(proof).toContain("'chat_text_generation'")
    expect(proof).toContain("'text_to_image'")
    expect(proof).toContain("'provider_auto_selection'")
