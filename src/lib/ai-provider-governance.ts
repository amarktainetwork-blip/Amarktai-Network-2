...
export const AI_PROVIDER_GOVERNANCE: readonly ProviderGovernanceEntry[] = [
  ...
  },
  {
    ...
    capabilities: ['image_generation', 'video_generation', 'music_generation', 'adult_image'],
    coveredByGenX: false,
    wired: true,
    showInPrimarySetup: true,
...
export function getRuntimeProviderGovernance(): ProviderGovernanceEntry[] {
  return AI_PROVIDER_GOVERNANCE.filter((entry) => entry.status !== 'proposed')
}

export function getPrimarySetupProviders(): ProviderGovernanceEntry[] {
  return AI_PROVIDER_GOVERNANCE.filter((entry) => entry.showInPrimarySetup)
}
