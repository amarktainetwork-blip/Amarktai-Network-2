...
const CAPABILITY_SPECS: CapabilitySpec[] = [
  ...
  },
  {
    ...
    requiresAdultGate: false,
    requiresDedicatedEndpoint: false,
    providerRequiredEnv: { genx: ['GENX_MUSIC_MODEL'] },
    proofNextAction: 'Set GENX_MUSIC_MODEL for GenX, then run live Pack A music proof.',
  },
  {
...
function providerConfigBlocker(spec: CapabilitySpec, providerIds: string[]): string {
  if (spec.capabilityId === 'music_generation') {
    return 'GenX music audio requires GENX_MUSIC_MODEL with GenX credentials.'
  }
  return providerIds
    .map((id) => {
...
function buildEntry(
  ...
  if (spec.capabilityId.startsWith('adult_')) {
    return {
      ...
      nextAction: 'Adult capabilities are deferred from active V1 readiness and proof.',
    }
  }
