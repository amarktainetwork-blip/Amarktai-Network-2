...
export async function getRepoWorkbenchStatus() {
  const [gitInstalled, ghInstalled, githubToken, writable, genx, groq, mimo, together] = await Promise.all([
  ...
  return {
    ...
    directProviderAvailable: groq || mimo || together,
