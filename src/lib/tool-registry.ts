...
export async function getToolRegistry(): Promise<{
  ...
  const tools = [
    ...
    entry({ id: 'firecrawl.scrape', label: 'Firecrawl scrape', category: 'research', enabled: configured.has('firecrawl'), wired: true, permission: 'write', approvalRequired: false, requiredConfig: ['firecrawl'], endpoint: '/api/admin/app-intelligence/crawl', lastError: configured.has('firecrawl') ? null : 'Firecrawl key not configured.' }),
    entry({ id: 'webdock.health', label: 'Webdock health', category: 'vps', enabled: configured.has('webdock'), wired: true, permission: 'read', approvalRequired: false, requiredConfig: ['webdock'], endpoint: '/api/admin/vps', lastError: configured.has('webdock') ? null : 'Webdock key not configured; local health may still be available.' }),
