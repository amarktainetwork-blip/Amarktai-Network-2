...
export async function getToolRegistry(): Promise<{
  ...
  const imageReady = capability('Image Generation')?.status === 'READY'
  const videoReady = capability('Video Generation')?.status === 'READY'
  const ttsReady = capability('Voice TTS')?.status === 'READY'
