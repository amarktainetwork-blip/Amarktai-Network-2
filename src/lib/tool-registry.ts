...
export async function getToolRegistry(): Promise<{
  ...
  const tools = [
    ...
    entry({ id: 'firecrawl.scrape', label: 'Firecrawl scrape', category: 'research', enabled: configured.has('firecrawl'), wired: true, permission: 'read', approvalRequired: false, requiredConfig: ['firecrawl'], endpoint: '/api/admin/research/url', lastError: configured.has('firecrawl') ? null : 'Firecrawl key not configured.' }),
